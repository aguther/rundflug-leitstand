...
| QS-09 | Betreiber ändert No-Show-Frist und Boardingzeit am Veranstaltungstag | Änderung wirkt sofort über die normale Kommandopipeline, auditiert, ohne Deployment | Kapitel 8.11, `scripts/verify_master_data.mjs` |
...
| QS-17 | Ein Branch-Commit oder Pull Request beschädigt Worker-Laufzeit, D1-Kernablauf, Restore oder Dokumentation | Jeder Branch-Push erhält getrennte parallele CI-Jobs und ein Sonar-Branch-Gate; interne Pull Requests erhalten zusätzlich eine native PR-Analyse. Der Sonar-Scanner wird aus dem npm-Lockfile installiert, V1-Suiten laufen zwischen vier Runnern parallel, je Runner aber strikt seriell | `.github/workflows/ci.yml`, `docs/operations/ci-cd-stabilitaet.md`, ADR-0057 |
| QS-18 | Operations-Board mit 300 Umläufen und 1.200 synthetischen Tickets sowie Stammdaten-Template mit 200 Flugzeugen | Die 14 Operations-Kernabfragen laufen in einem D1-Batch, die Registrierungsprüfung in einer Abfrage und die reine Boardprojektion bleibt unter 500 ms | `apps/worker/src/operations-read-query-plan.ts`, `apps/worker/src/operations-read-service.test.ts`, `apps/worker/src/operations-response-projector.ts`, `apps/worker/src/operations-routes.test.ts`, `apps/worker/src/admin-master-data-template.test.ts`, `docs/verification/operations-read-performance-2026-08-11.md` |
...
| QS-21 | Eine Rollenansicht erhält neue Logik oder Styles | Lazy Route-Grenzen bleiben erhalten; globale CSS, Flight-Line-CSS, Admin-/Main-Entry, größter JavaScript-Chunk und PWA-Precache besitzen mindestens zehn Prozent Reserve zu unveränderten Hard Limits; jede Route bleibt innerhalb des Zwei-Prozent-Ratchets und die operative Precache-Policy gilt | `npm run web:assets:report`, `npm run web:assets:verify`, `scripts/data/web-asset-baseline.json`, ADR-0055, `docs/verification/web-asset-headroom-2026-08-16.md` |
| QS-22 | Cloudflare-Runtime oder Compatibility-Date driften; ein D1-Baseline-Neuaufbau wird vorbereitet | Der monatliche Workflow fordert spätestens nach 45 Tagen eine bewusste Aktualisierung, prüft Runtime, Bindings, Observability, Health/Meta und Logdatenschutz; das D1-CLI bleibt ohne zielgebundene Bestätigung read-only | `npm run cloudflare:maintenance`, `npm run cloudflare:recreate-d1`, `.github/workflows/cloudflare-maintenance.yml`, ADR-0049 |
| QS-23 | Ein Werksreset trifft auf mehrere zehntausend Planläufe und Prognose-Snapshots oder wird während einer Bulk-Phase unterbrochen | Begrenzte D1-Transaktionen löschen die Historie vollständig; bei Unterbrechung bleiben Admin-Zugang und Wurzeldaten für die Wiederholung erhalten, und der Beleg entsteht erst im finalen Commit | `apps/worker/src/factory-reset.test.ts`, `scripts/verify_factory_reset.mjs`, ADR-0050 |
| QS-24 | Drei parallele 72-Stunden-Veranstaltungen erzeugen im 30-Sekunden-Takt Planungsläufe für je 300 Umläufe | Je aktiver Veranstaltung bleiben höchstens 24 Stunden plus Boundary in D1; kalte und heiße Mengen sind lücken- und dublettenfrei, terminal bleibt kein Detailrest, Forecast-p95 verschlechtert sich höchstens zehn Prozent und bleibt unter zwei Sekunden | `scripts/verify_planning_history_scale.mjs`, `apps/worker/src/planning-history-compaction.test.ts`, ADR-0053 |
| QS-25 | Eine isolierte Wiederherstellung verbindet portables Backup mit mehreren kalten Segmenten | ZIP-, Datei- und Mengenhashes stimmen, Boundary-Links werden chronologisch rekonstruiert, Fremdschlüssel und Replay sind gültig; eine manipulierte Datei stoppt den Restore | `scripts/verify_planning_history_restore.mjs`, `scripts/verify_analysis_replay.mjs`, ADR-0053 |
| QS-26 | Ein geprüfter `main`-Commit enthält eine D1-Migration oder Cloudflare antwortet beziehungsweise aktiviert die neue Worker-Version vorübergehend uneinheitlich | Der Preflight verweigert neue oder abweichende Ressourcen; nur per SHA-256 unveränderte Online-Migrationen laufen nach Time-Travel-Bookmark und portablem Backup. Ausschließlich 429/5xx, Timeouts und Verbindungsabbrüche werden begrenzt wiederholt. Code und Backup-Token-Hash entstehen in einer Version; `/api/meta` muss abschließend zweimal hintereinander exakt den Commit melden | `scripts/cloudflare_preflight.mjs`, `scripts/cloudflare_deploy.mjs`, `scripts/lib/deployment-verification.test.ts`, `apps/worker/src/deployment-backup-routes.test.ts`, ADR-0057 |
