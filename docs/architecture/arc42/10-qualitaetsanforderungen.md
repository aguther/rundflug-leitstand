...
| QS-26 | Ein geprüfter `main`-Commit enthält eine D1-Migration oder Cloudflare antwortet vorübergehend nicht | Der Preflight verweigert neue oder abweichende Ressourcen; nur per SHA-256 unveränderte Online-Migrationen laufen nach Time-Travel-Bookmark und portablem Backup. Ausschließlich 429/5xx, Timeouts und Verbindungsabbrüche werden begrenzt wiederholt; `/api/meta` muss abschließend exakt den Commit melden | `scripts/cloudflare_preflight.mjs`, `scripts/cloudflare_deploy.mjs`, `apps/worker/src/deployment-backup-routes.test.ts`, ADR-0057 |
