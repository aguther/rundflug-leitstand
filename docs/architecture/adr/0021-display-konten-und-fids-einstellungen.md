...
einheitliches, responsives Standardprofil, langlebige pseudonyme Display-Konten und ergänzenden
Zugriff für Administratoren freigegeben.

## Entscheidung

- `DISPLAY` ist eine reine Anzeigerolle. Ihre einzige Start- und Zielansicht ist `/fids`; alle
  übrigen geschützten Event-APIs und App-Ziele lehnen diese Rolle ab. `ADMIN` darf FIDS und die
  kontobezogenen FIDS-Einstellungen zusätzlich verwenden; seine Startseite bleibt `/admin`.
  Kassen-, Flight-Line- und Flight-Director-Konten erhalten keinen Zugriff auf FIDS.
...
Sitzungswiderruf unmittelbar greifen. Administratoren können FIDS für Einrichtung und Kontrolle
mit ihrer unverändert 16 Stunden gültigen Sitzung öffnen. Portable Backups bleiben frei von Konten,
Sitzungen und Anzeigepräferenzen; eine vollständige Wiederherstellung dieser Daten erfolgt über D1
Time Travel.
