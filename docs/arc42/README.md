...
freigegebenen ADRs unter `docs/adr/` und die automatisierten Prüfungen. Diese Dokumentation fasst
zusammen, ordnet ein und verweist – sie ersetzt keine dieser Quellen.
...
Kapitel, rendert alle Mermaid-Diagramme mit der exakt gepinnten lokalen Mermaid-Version und erzeugt
ein druckfertiges A4-PDF mit Titelblatt, verlinktem Inhaltsverzeichnis, Kopf- und Fußzeilen sowie
Seitenzahlen. Pandoc, LaTeX, Docker und ein ungepinnter `npx`-Download werden nicht benötigt.
...
werden darin repository-relativ umgeschrieben. Der PDF-Build verwendet dauerhaft klickbare Links auf
die entsprechenden Dateien im `main`-Branch des GitHub-Repositories. Die erzeugten Dateien sind
abgeleitete Artefakte und werden nicht versioniert.
...
- Architekturentscheidungen werden zuerst als ADR unter `docs/adr/` festgehalten und hier nur
  zusammengefasst.
- Detailtiefe zu Fachmodell, Prognose, Dispatch und technischen Schulden bleibt in
  `docs/architecture/`; diese Dokumentation verweist darauf, statt sie zu duplizieren.
