...
- `requirements-v1.10.0.md` und `.yaml` sind die einzige aktuelle Releasefassung. Sie
  konsolidieren alle fortgeltenden Anforderungen, die kanonischen Rollenbegriffe, den sicheren
  Reset-/Setup-Ablauf, Rollenblätter und den portablen Cloudflare-Neuaufbau.
...
- `requirements-v1.10.0.md` und `.yaml` enthalten alle 319 aktuell fortgeltenden Anforderungen:
  den vollständigen Basiskatalog, 99 migrierte Deltas aus 1.5 bis 1.9.1 und das 1.10.0-Delta.
- `traceability-v1.10.0.csv` ist die vollständige aktuelle Traceability; `traceability.csv` bleibt
...
1.10.0-Anforderungen; `npm run docs:requirements:check` lehnt manuelle Abweichungen ab.
