  ...
  const addRule = () => {
    ...
    onChange({
      ...
      recurringRules: [
        ...existingRules,
        {
          key: `sim-rule-${String(existingRules.length + 1).padStart(3, "0")}`,
      ...
      <section className="sim-editor-card">
        <header className="sim-editor-section-heading">
          <div>
            ...
            <p>
              Prognosewirksame Pausen und Tankvorgänge nach bestätigten Umläufen oder
              Betriebsminuten.
            </p>
          </div>
          <span className="sim-parameter-tag sim-parameter-tag--simulation">Simulation</span>
        ...
        <div className="sim-plan-list">
          {(config.recurringRules ?? []).length === 0 ? (
            <p className="sim-editor-hint">Noch keine wiederkehrende Regel vorhanden.</p>
          ) : null}
            ...
            return (
              <article className="sim-plan-entry" key={rule.key}>
                <header>
                  ...
                  <button
                    aria-label={`${rule.key} deaktivieren`}
                    onClick={() =>
                      onChange({
                        ...config,
