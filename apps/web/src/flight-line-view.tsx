...
function operationalSummaryPresentation(
...
function rotationTicketGroupIds(selectedQueueGroupIds: string[], rotation: Rotation): string[] {
...
function callNextRecommendationPayload(
...
function plannedResourceGroupStatus(
  plan: PlannedOperation,
  activate: boolean,
...
function QueueGroupPassengerSummary({ group }: Readonly<{ group: QueueGroup }>) {
...
function LegacyAircraftActions({
  aircraft,
  canManageAircraft,
  onPause,
  onSetState,
...
function RotationOvertimeNotice({
  rotation,
  timeZone,
...
export function FlightLineView() {
  ...
  const { summary: operationalSummary, tone: operationalSummaryTone } =
    operationalSummaryPresentation(board, selectedOperationalResourceGroup);
  ...
  async function advance(
    ...
    try {
      ...
      if (selectedAction.command === "CALL_NEXT") {
        ...
        const ticketGroupIds = rotationTicketGroupIds(selectedQueueGroupIds, selectedRotation);
  ...
  async function confirmPlannedOperation(plan: PlannedOperation, activate: boolean) {
    ...
    if (plan.effectMode === "SLOWDOWN") {
      await confirmPlannedSlowdown(plan, activate);
      return;
    }
    ...
    try {
      await sendCommand(
        plannedScopedCommand(plan, activate, expectedReviewAt, board.event.version),
        deviceTokenFor(FLIGHT_LINE_DEVICE_ID),
      );
      setMessage(
        activate
  ...
  function renderLegacyQueueGroupSelector() {
    if (!selectedAircraft || compatibleQueueGroups.length === 0) return null;
    return (
      <section className="queue-group-selector" aria-labelledby="queue-groups-title">
        ...
        <div className="queue-group-options">
          {compatibleQueueGroups.map((group) => {
            ...
            return (
              <article
                className={selectedGroup ? "queue-group-option selected" : "queue-group-option"}
                key={group.id}
              >
                <label>
                  <input
                    ...
                    onChange={(event) => toggleLegacyQueueGroup(group, event.target.checked)}
                    type="checkbox"
                  />
                  <span>
                    <strong>
                ...
                <div className="queue-group-actions">
                  <button
                    onClick={() => void setGroupAttendance(group.id, group.status !== "PRESENT")}
                    type="button"
                  >
  ...
  function renderLegacyRotationQueue() {
    const rows = aircraftRotations?.map((rotation) => {
      ...
      return (
        <div className="queue-row-wrap" key={rotation.id}>
          <button
            ...
            onClick={() => selectLegacyRotation(rotation, false)}
            type="button"
          >
          ...
          <button
            ...
            onClick={() => selectLegacyRotation(rotation, true)}
            type="button"
          >
            Disposition
          </button>
    ...
    if (selectedAircraft && aircraftRotations?.length === 0) {
      return <p>Für dieses Flugzeug ist aktuell keine passende Fluggruppe offen.</p>;
    }
    if (!selectedAircraft) return <p>Kein aktives Flugzeug verfügbar.</p>;
    return rows;
  }

  function renderOperationalConsole() {
  ...
  function renderLegacyAircraftSelector() {
    return (
      ...
      <section className="supervisor-aircraft-summary">
        ...
        <div className="supervisor-aircraft-actions">
          <LegacyAircraftActions
            ...
            onPause={() => openAircraftPauseDialog()}
            onSetState={(state) => void setFlightLineAircraftState(state)}
          />
        </div>
      </section>
    );
  ...
  function renderSelectedRotationDetails() {
  ...
  function renderAttendanceDecision() {
    ...
    return (
      <section className="attendance-decision">
        ...
        {!noShowReady ? (
          <p>No-Show ist erst nach {board?.event.noShowAfterMinutes ?? 10} Minuten verfügbar.</p>
        ) : null}
  ...
  function renderDispositionPanel() {
    if (!dispositionOpen || !selected) return null;
    return (
        ...
        ["FLIGHT_DIRECTOR", "ADMIN"].includes(board?.currentDeviceRole ?? "") ? (
          <section>
            ...
            <div className="compact-stepper">
              ...
              <button
                onClick={() =>
                  setDispositionCapacity((value) => Math.min(selected.baselineCapacity, value + 1))
                }
                type="button"
              >
                +
        ...
        {["DRAFT", "CALLED"].includes(selected.status) ? (
          <section>
            ...
            <button
              ...
              onClick={() => void moveTicketGroup(selected.ticketGroupId, moveTargetId, moveReason)}
              type="button"
            >
        ...
        >
          ...
          <section className="flight-workspace">
            <div className="queue-list">
              ...
              {renderLegacyQueueGroupSelector()}
              {renderLegacyRotationQueue()}
            </div>
            <div className="rotation-detail">
              {renderSelectedAircraftSummary()}
              {renderSelectedRotationDetails()}
            </div>
