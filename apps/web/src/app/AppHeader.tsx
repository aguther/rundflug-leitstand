...
function headerModeClass(publicView: boolean, fidsView: boolean): string {
  if (publicView) return "app-header--public";
  return fidsView ? "app-header--fids" : "app-header--compact";
}

function revisionCopyMessage(status: RevisionCopyStatus): string {
  if (status === "copied") return "Kopiert";
  return status === "error" ? "Kopieren nicht möglich." : "";
}
...
export function AppHeader({
  ...
  function renderBrand() {
    if (internalOperationalView || publicView) {
      return <div className="app-brand">{brandContent}</div>;
    }
    return (
  ...
  function renderConnectionStatus() {
    if (kiosk || publicView || !session) return null;
    return (
      <output
  ...
  function renderFidsEventButton() {
    if (!fidsView || !session || kiosk || publicView || !eventLabel) return null;
    return (
      <button
        className="app-event"
  ...
  function renderFidsInfoMenu() {
    if (!fidsView || kiosk) return null;
    return (
  ...
  function renderAccountMenu() {
    if (!session || kiosk || publicView) return null;
    return (
      <details
  ...
  return (
    <>
      <header className={`app-header ${headerModeClass(publicView, fidsView)}`}>
        {renderBrand()}
        {renderConnectionStatus()}
        {renderViewSwitcher()}
        {renderFidsEventButton()}
        {renderFidsInfoMenu()}
        ...
        <div className="app-about-dialog">
          ...
          <div className="app-about-revision">
            ...
            {sourceRevision.known ? (
              ...
              >
                {revisionCopyMessage(revisionCopyStatus)}
              </small>
            ) : null}
          </div>
        </div>
