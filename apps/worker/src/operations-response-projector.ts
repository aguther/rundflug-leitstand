...
import type { OperationsReadModels } from "./operations-read-service";
import {
  activePilot,
  aircraftProductOverrideFor,
  currentDispatchValue,
  dispatchPlanProjection,
  effectiveRotationTurnaroundProfile,
  forecastProfileAircraftId,
  forecastWindow,
  forecastWindowCenterMs,
  precallDecisionProjection,
} from "./operations-route-projection";
import {
...
export interface OperationsResponseProjectionInput {
  eventId: string;
  eventRow: StoredEventRow;
  device: AuthorizedDevice;
  readModels: OperationsReadModels;
  forecastReadAt: string;
  nowMs: number;
}

export function buildOperationsResponse(input: OperationsResponseProjectionInput) {
  const { eventId, eventRow, device, readModels } = input;
  const {
    products,
  ...
  const forecastReadAt = input.forecastReadAt;
  ...
  return {
    ...
    products: products.results.map((product) => {
      ...
      return {
        ...
        saleRecommended:
          ...
          eventRow.operational_interrupted !== 1 &&
          (product.sale_closes_at === null || Date.parse(product.sale_closes_at) > input.nowMs) &&
          (!eventRow.sale_opens_at || Date.parse(eventRow.sale_opens_at) <= input.nowMs),
    ...
    plannedOperations: plannedOperationRows.results.map((plan) => ({
      ...
      status:
        plan.status === "PLANNED" &&
        ((plan.latest_start_at !== null && Date.parse(plan.latest_start_at) <= input.nowMs) ||
          (plan.start_mode === "AFTER_CURRENT_ROTATION" &&
