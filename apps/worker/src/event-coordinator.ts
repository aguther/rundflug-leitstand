...
import {
  ...
  type ForecastRecalculationResult,
...
import { sendRotationPushNotifications, sendTicketGroupRecallPushNotifications } from "./web-push";
...
interface DispatchRecommendationAircraft {
  id: string;
...
interface DispatchRecommendationPlanningRow {
  rotation_id: string;
  created_at: string;
...
export class EventCoordinator extends DurableObject<Env> {
  ...
  private async handleDispatchRecommendationLease(request: Request, url: URL): Promise<Response> {
    ...
    if (ownedLease) {
      if (ownedLease.aircraft_id === aircraft.id) {
        if (await this.dispatchRecommendationLeaseIsRelevant(ownedLease, aircraft)) {
        ...
        await this.env.DB.batch([
          ...
          ).bind(nowIso, ownedLease.id, ownedLease.version),
          this.env.DB.prepare(
            `INSERT INTO operational_events
              (id, operation_day_id, event_type, occurred_at, device_id, aggregate_type,
               aggregate_id, aggregate_version, payload_json)
    ...
    if (canonicalPlanReplanned) {
      await this.forecastTimelineService.recalculateForecastTimelines({
        eventId,
  ...
  private async runManualAnalysisCapture(
    ...
    try {
      const captured = await this.forecastTimelineService.recalculateForecastTimelines({
