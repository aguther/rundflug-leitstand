...
export class FleetAdministrationCommandService {
    ...
    if (command.type === "UPSERT_PILOT" || command.type === "SET_PILOT_PAUSE") {
      const plan =
        command.type === "SET_PILOT_PAUSE"
          ? await this.preparePilotPause(command, now)
          : await this.preparePilotUpsert(command, now);
      if (plan instanceof Response) return plan;
      ({ aggregateType, aggregateId, eventType, auditPayload } = plan);
      statements.push(...plan.statements);
    } else {
      const plan = await this.prepareAircraftCommand(command, now);
      if (plan instanceof Response) return plan;
      ({ aggregateType, aggregateId, eventType, auditPayload } = plan);
      statements.push(...plan.statements);
    }
  ...
  private async preparePilotPause(
  ...
  private async preparePilotUpsert(
  ...
  private async prepareAircraftCommand(
